...
def _signup_error_message(exc: Exception) -> str:
    raw = str(exc).lower()
    if "already been registered" in raw or "already registered" in raw or "user already" in raw:
        return (
            "This email is already registered in Supabase Auth. "
            "Try logging in. If you never completed signup, a past attempt may have reserved this email — use Login or password reset."
        )
    if "duplicate key" in raw or "unique constraint" in raw:
        return (
            "This email or account already exists. Try logging in instead."
        )
    return str(exc)
...
async def signup(request: SignupRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=_signup_error_message(e))

    if not auth_response.user:
        raise HTTPException(
            status_code=400,
            detail="Signup did not create a user. Check Supabase Auth settings (e.g. email confirmation) or try again.",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=(
                "Auth account was created but saving your profile failed. "
                "Try logging in with this email — if login works, you are registered. "
                f"Details: {_signup_error_message(e)}"
            ),
        )
